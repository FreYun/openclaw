...
当主题段涉及某个板块的逻辑阐述时，不能只看今天涨跌，必须拉该板块的历史行情来建立上下文。
...
# 行业板块历史日K（东方财富）
# symbol 为板块名称，如"半导体""银行""电力设备"
df = ak.stock_board_industry_hist_em(symbol="半导体", period="日k", start_date="20260101", end_date="20260320")
print(df.tail(30))  # 近30个交易日走势

# 概念板块历史日K
df = ak.stock_board_concept_hist_em(symbol="算力租赁", period="日k", start_date="20260101", end_date="20260320")
print(df.tail(30))

# 实用分析角度：
# 1. 该板块近5/10/20日累计涨跌幅 → 判断是启动初期还是高位
# 2. 区间最高/最低点及当前位置 → 历史分位
# 3. 成交额变化趋势 → 资金是在进场还是离场
# 4. 对比同期大盘指数表现 → 是否跑赢/跑输大盘
```
...
1. **搜研报（skill-gateway）：** 用 `skill-gateway.search_report` 搜索券商研报，语义匹配，找近期券商对该板块大趋势的判断（供需格局、景气周期、政策催化等）。调用方式：`npx mcporter call "skill-gateway.search_report(query='{板块名}', top_k=3, days_ago_limit=30)"`
2. **查知识星球：** 用 `zsxq-reader` skill（`Read skills/zsxq-reader/SKILL.md` 后执行）在已加入的星球中搜索该板块关键词，提取大V对板块趋势的解读、逻辑链、核心标的等
3. **搜产业新闻：** WebSearch `"{板块名}" 产业 最新动态`，找近期的产业面变化（订单、产能、价格、政策）
