...
def build_universe(t_date: str) -> tuple:
    """构造候选股池 = T 日热门行业 (top 3) 的成分股合集。

    Returns:
        (codes: list, hot_industries: list, code_to_industry: dict)
    """
    zt_df = fetch_zt_pool(t_date)
    if zt_df is None:
        logging.warning(f"T 日 {t_date} 涨停池为空, 无法确定热门行业")
        return [], [], {}

    hot = derive_hot_industries(zt_df, top_n=3)
    if not hot:
        return [], [], {}

    logging.info(f"T 日热门行业 top 3: {[h['name'] for h in hot]}")

    all_codes = set()
    code_to_industry = {}
    for h in hot:
        cons = fetch_industry_constituents(h["name"])
        logging.info(f"  {h['name']}: {len(cons)} 只成分")
        for c in cons:
            all_codes.add(c)
            # 第一个匹配到的行业记录为该股的归属 (避免覆盖)
            if c not in code_to_industry:
                code_to_industry[c] = h["name"]

    # 同时把 T 日涨停池的所有股加入 universe (确保不漏掉热门股)
    for _, row in zt_df.iterrows():
        c = str(row["代码"]).zfill(6)
        all_codes.add(c)
        if c not in code_to_industry:
            code_to_industry[c] = row.get("所属行业", "未知")

    return sorted(all_codes), hot, code_to_industry


def fetch_universe_klines(codes: list, t_date: str) -> dict:
    """为 universe 拉 35 个交易日的 K 线 (research-mcp 批量)。"""
    start_date_str = shift_trading_days(t_date, -35)
    end_date_str = t_date
    start = start_date_str.replace("-", "")
    end = end_date_str.replace("-", "")
    logging.info(f"批量 K 线: {len(codes)} 只 from {start_date_str} to {end_date_str}")
    return fetch_klines_batch(codes, start, end)
...
def _build_name_map(t_date: str, codes: list) -> dict:
    """补全股票名 — 优先从 T 日涨停池, 兜底用代码。

    简单实现: 重新拉一次涨停池 (反正前面已经拉过, 用 lru_cache 化简或直接复用)
    """
    name_map = {}
    zt_df = fetch_zt_pool(t_date)
