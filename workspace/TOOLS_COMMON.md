...
- **Always close tabs when done (`browser close`)** — stale tabs cause renderer to spike CPU
...
## Financial Data: Skill Gateway

Distributed via **skill-gateway** (`http://localhost:18080`) with role-based access:

| Role | Bots | Scope |
|------|------|-------|
| `full_access` | bot7, bot8, bot2 | All (Tushare, research-mcp, sector analysis, etc.) |
| `fund_advisor` | bot3, bot5 | Funds, macro, market overview |
| `content_creator` | bot1, bot4, bot6, bot9, bot10 | Basic market quotes |

Call: `npx mcporter call "skill-gateway.tool_name(...)"` — gateway filters available tools by bot identity.

Data sources: **Tushare** (A-share quotes/financials/northbound flows), **research-mcp** (TTFund/Wind/funds/bonds/macro/commodities).
...
2. **skill-gateway** → financial data (role-gated)
