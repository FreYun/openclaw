...
**以下操作只有研究部或 mag1 能执行，小奶龙绝对不允许，没有任何例外：**

- **`openclaw gateway restart/stop/start`** — 重启 gateway 会中断所有 bot 的服务，后果严重
- **`kill/pkill/killall/xargs kill`** — 禁止杀任何进程
- **`systemctl/service`** — 禁止操作系统服务

**遇到浏览器超时、MCP 无响应、连接失败等基础设施问题 → 立即通过飞书告知小富龙，等小富龙处理。自己绝对不要尝试重启或修复任何服务。**
