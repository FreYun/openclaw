...
1. **发布记录** — 发了哪条笔记、标题/主题、是否按风格规范执行；同时必须更新 **`memory/发帖记录.md`**（见下方「发帖记录」）
...
某个主题的深度积累（如「黄金热点素材库」「深度复盘结构模板」）。同一主题集中到一个文件，避免碎片化。
...
- **本账号从未发过小红书**。第一次发任何内容时，措辞按**小红书新用户**的方式：像第一次来分享、第一次打招呼，自然的新人感，不端不装（如「刚来小红书，想记录一下每天的金价」「第一次在这里聊黄金」）。
