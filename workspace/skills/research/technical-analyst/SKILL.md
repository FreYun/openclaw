...
如果环境中存在 `research-mcp` 或 `stock_research` 工具，可在分析图表前调用以获取辅助数据：

- **用途**：验证图表中读取到的价格水平、获取最新收盘价、补充成交量数据
- **调用方式**：`stock_research(symbol="[标的代码]")` 或等效工具
- **注意**：辅助数据仅用于校验图表读数，分析结论仍以图表视觉数据为主要依据
- **工具不可用时**：直接跳过此步骤，仅基于图表图片进行分析；不因工具缺失而中断流程
