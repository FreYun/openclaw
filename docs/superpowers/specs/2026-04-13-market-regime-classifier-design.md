...
| **情绪评分**（0-100 复合指标） | >70 | 55-70 | 40-55 | 25-40 | <25 | 情绪温度计.情绪评分 |
...
3. **情绪塌方**：当日情绪评分 ≤ 20（满分 100），**或** 涨停数减去跌停数 ≤ −40
...
|---|---|---|---|---|---|
| 2026-04-13 | MA:+1, 涨跌:0.55, 涨停差:25, 评分:58, 连板:3, 量比:1.05 | 1,0,1,1,0,0 | +3 | 中性震荡 | - |
```
...
4. **阈值需要校准**：初始阈值是根据经验设定，上线后应每月看 `regime-log.md` 校准一次，尤其是"情绪评分"和"连板高度"两个维度（A 股情绪波动大）
