...
```bash
npx mcporter call "xiaohongshu-mcp.get_both_login_qrcodes()"
```
...
```bash
npx mcporter call "xiaohongshu-mcp.get_creator_login_qrcode()"
```
...
```bash
npx mcporter call "xiaohongshu-mcp.delete_cookies()"
```
...
npx mcporter call "xiaohongshu-mcp.search_feeds(keyword: '黄金')"
