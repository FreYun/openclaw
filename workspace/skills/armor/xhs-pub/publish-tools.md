...
# 服务名 = "xhs-" + account_id
# 简单参数（无特殊字符）
npx mcporter call --timeout 180000 "xhs-bot5.publish_content(account_id: 'bot5', title: '...', ...)"

# 含特殊字符时必须用 --args + JSON 文件（见 publish-pipeline.md 步骤7）
npx mcporter call --timeout 180000 xhs-bot5.publish_content --args "$(cat args.json)"
```

**超时必设**：每次调用都加 `--timeout 180000`，不省略。
