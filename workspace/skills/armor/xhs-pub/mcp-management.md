...
单进程多租户，所有 bot 直连 xiaohongshu-mcp（`:18060`），URL path 区分 bot 身份。MCP 内部有 per-bot 互斥锁，同一 bot 串行、跨 bot 并行：

```
http://localhost:18060/mcp/bot7   ← bot7 的入口
http://localhost:18060/mcp/bot1   ← bot1 的入口
```
...
> **印务局禁止自行重启 MCP 服务。** kill MCP 会中断所有 bot 正在执行的浏览器操作。
> 遇到 MCP 异常时：上报 mag1，等待管理员处理。

### 安全铁律

- **禁止 `pkill -9 -f "chrome.*xhs-profiles"`** — 通配符强杀会杀掉所有 bot 的 Chrome，导致 cookie 丢失
- **禁止 `lsof -ti:18060 | xargs kill`** — 会中断其他 bot 的发布操作
- 重启 MCP 服务不影响 Chrome cookie（它们在 profile 目录中持久化），但会中断正在执行的操作
...
```bash
# 必须用 xhs-botN，不是 xiaohongshu-mcp
npx mcporter call --timeout 180000 "xhs-botN.check_login_status(account_id: 'botN')"
```
