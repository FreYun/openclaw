...
| [养号流程.md](养号流程.md) | 收到养号调度指令时 — 搜索→点赞→评论→汇报的完整 SOP |
...
账号从未发过小红书时（`memory/posts/` 为空），按**新用户**方式措辞。后续发文**建立在前一次基础上**，有连续性——动笔前用 `mem0_search` 查过往发帖，确认话题承接。

### 发文后

1. 更新当日 `memory/YYYY-MM-DD.md`
2. 被打回时更新 `memory/写稿经验.md`
