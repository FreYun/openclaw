...
Collect latest market data using **zhipu MCP search** (内置 web_search 已禁用，一律用 zhipu MCP):

Search queries to run in parallel:
1. `"S&P 500 NASDAQ Dow Jones latest price today"` — US indices
2. `"Nikkei 225 Shanghai Composite Hang Seng latest"` — Asian indices
3. `"EUR/USD USD/JPY forex rates today"` — Forex
4. `"WTI crude oil gold silver price today"` — Commodities
5. `"US Treasury yield 10-year VIX index today"` — Rates & volatility

Also use **browser** to browse financial sites for real-time data where search results are insufficient (e.g. investing.com, markets.ft.com).
