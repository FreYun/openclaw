...
**你是 bot7，调用所有 xiaohongshu-mcp 工具时必须传 `account_id: "bot7"`，不可省略。**
...
```bash
curl -s http://localhost:18067/health
```
...
```bash
nohup /home/rooot/MCP/xiaohongshu-mcp/xiaohongshu-mcp -headless=true -port=:18067 > /tmp/xiaohongshu-mcp.log 2>&1 &
```

启动后等 2 秒再确认：

```bash
sleep 2 && curl -s http://localhost:18067/health
```
...
```bash
npx mcporter call "xiaohongshu-mcp.get_both_login_qrcodes(account_id: 'bot7')"
```
...
1️⃣ 小红书主站", media="/home/rooot/.openclaw/media/xhs-qr-bot7.png")
message(action="send", message="2️⃣ 小红书创作者平台", media="/home/rooot/.openclaw/media/xhs-creator-qr-bot7.png")
```

### 情况 B：仅主站未登录 → 获取主站二维码

```bash
npx mcporter call "xiaohongshu-mcp.get_login_qrcode(account_id: 'bot7')"
```

```
message(action="send", message="请用小红书 App 扫码登录主站 👇", media="/home/rooot/.openclaw/media/xhs-qr-bot7.png")
```

### 情况 C：仅创作者平台未登录 → 获取创作者平台二维码

```bash
npx mcporter call "xiaohongshu-mcp.get_creator_login_qrcode(account_id: 'bot7')"
```

```
message(action="send", message="请用小红书 App 扫码登录创作者平台 👇", media="/home/rooot/.openclaw/media/xhs-creator-qr-bot7.png")
```

### 重置登录

```bash
npx mcporter call "xiaohongshu-mcp.delete_cookies(account_id: 'bot7')"
```
...
所有工具均需传 `account_id: "bot7"`：

```bash
npx mcporter call "xiaohongshu-mcp.list_notes(account_id: 'bot7')"
npx mcporter call "xiaohongshu-mcp.publish_content(account_id: 'bot7', title: '标题', content: '内容', images: ['url'])"
```
...
| `get_login_qrcode` | 获取主站登录二维码（自动保存 `/home/rooot/.openclaw/media/xhs-qr-bot7.png`）|
| `get_creator_login_qrcode` | 获取创作者平台登录二维码（自动保存 `/home/rooot/.openclaw/media/xhs-creator-qr-bot7.png`）|
...
```bash
npx mcporter call "xiaohongshu-mcp.publish_content(account_id: 'bot7', title: '春日出游指南', content: '...正文...', text_to_image: true)"
```

### 定时发布示例

```bash
npx mcporter call "xiaohongshu-mcp.publish_content(account_id: 'bot7', title: '..', content: '..', images: ['/path/img.jpg'], schedule_at: '2025-12-01T10:00:00+08:00')"
```
