...
- **MX_APIKEY**：妙想Skills页面获取的API密钥，需保密。**按 bot 独立管理**，每个 bot 在自己的 workspace 目录下放一份 `.mx_apikey`（纯文本单行，chmod 600）。
...
**所有调用必须走 wrapper [`skills/mx-moni/run.sh`](run.sh)，不要直接 `python3 mx_moni.py`。**

wrapper 的 key 查找顺序：

1. 环境变量 `$MX_APIKEY`（若已 export，优先使用，方便临时覆盖）
2. `$PWD/.mx_apikey`（当前 bot workspace 下的独立 key 文件，**推荐方式**）
3. 都没有 → 报错退出

调用时 bot 进程的 cwd 必须是 `~/.openclaw/workspace-botN/`（openclaw 默认如此），wrapper 会从这里读取 `.mx_apikey`。

首次配置示例（bot 管理员）：

```bash
# 在 bot 的 workspace 目录下写入独立 key（不要共用！）
echo -n "mkt_xxx_your_bot_specific_key" > ~/.openclaw/workspace-botN/.mx_apikey
chmod 600 ~/.openclaw/workspace-botN/.mx_apikey
```

> ⚠️ `.mx_apikey` 已加入 `.gitignore`，不会被提交。不要把 key 写到 SOUL/TOOLS/MEMORY 等 md 文件里。
...
bash skills/mx-moni/run.sh "查询持仓"
bash skills/mx-moni/run.sh "我的资金"
bash skills/mx-moni/run.sh "账户余额"
bash skills/mx-moni/run.sh "我的委托"
bash skills/mx-moni/run.sh "查询订单"

# 2. 买入操作
bash skills/mx-moni/run.sh "买入 600519 价格 1700 数量 100 股"       # 限价买入
bash skills/mx-moni/run.sh "市价买入 600519 100 股"                   # 市价买入
bash skills/mx-moni/run.sh "买 300059 19.4 100股"                     # 简写也支持

# 3. 卖出操作
bash skills/mx-moni/run.sh "卖出 600519 价格 1750 数量 100 股"      # 限价卖出
bash skills/mx-moni/run.sh "市价卖出 600519 100 股"                  # 市价卖出
bash skills/mx-moni/run.sh "卖 300059 19.5 100股"                     # 简写也支持

# 4. 撤单操作
bash skills/mx-moni/run.sh "撤单 260854300000078983"                 # 按委托编号撤单
bash skills/mx-moni/run.sh "一键撤单"                                 # 撤销当日所有未成交委托
```
...
**语法格式：**
```
bash skills/mx-moni/run.sh "买入 [股票代码] [价格] [数量] 股"
bash skills/mx-moni/run.sh "市价买入 [股票代码] [数量] 股"
```
...
**语法格式：**
```
bash skills/mx-moni/run.sh "卖出 [股票代码] [价格] [数量] 股"
bash skills/mx-moni/run.sh "市价卖出 [股票代码] [数量] 股"
```
...
**语法格式：**
```
bash skills/mx-moni/run.sh "撤单 [委托编号]"
bash skills/mx-moni/run.sh "一键撤单"
```
...
| 功能 | 触发词 | 使用示例 |
|------|--------|----------|
| 持仓查询 | `查询持仓`、`我的持仓`、`持仓情况` | `bash skills/mx-moni/run.sh 我的持仓` |
| 资金查询 | `查询资金`、`我的资金`、`账户余额`、`资金情况` | `bash skills/mx-moni/run.sh 我的资金` |
| 委托查询 | `查询委托`、`我的委托`、`订单`、`委托记录` | `bash skills/mx-moni/run.sh 我的委托` |
| 买入操作 | `买入`、`买进`、`建仓`、`市价买入` | `bash skills/mx-moni/run.sh 买入 600519 1700 100股` |
| 卖出操作 | `卖出`、`抛售`、`减仓`、`市价卖出` | `bash skills/mx-moni/run.sh 卖出 600519 1750 100股` |
| 撤单操作 | `撤单`、`撤销`、`一键撤单` | `bash skills/mx-moni/run.sh 撤单 260854300000078983` |
