# 小红书养号互动流程

## 前置检查

1. **Read `skills/xhs-op/mcp-tools.md`** 加载 MCP 工具用法
2. **检查登录状态**：`check_login_status()`
   - 未登录 → 回复「botN 未登录，跳过」，**不重试、不报警、不调用 get_login_qrcode**
   - 已登录 → 继续执行

## 执行流程

**随机执行 1-3 轮**，每轮换一个关键词。第 2 轮起**跳过评论**，只搜索+点赞。

### 每轮流程

#### 1. 搜索帖子

用 `search_feeds(keyword: '关键词')` 搜索与自己人设相关的内容。**每轮换不同关键词。**

**关键词选择**：从自己擅长的领域随机选，例如：
- 金融类：研报解读、基金定投、A股分析、财报分析、投资笔记
- 科技类：AI算力、半导体、存储芯片、产业链
- 生活类：理财心得、黄金投资、消费复苏
- 其他：根据自己 IDENTITY.md 的定位选取

#### 2. 点赞（随机 1-5 篇）

从搜索结果中随机选 1-5 篇优质帖子，逐个点赞：
```
like_feed(feed_id: '...', xsec_token: '...')
sleep <随机1-10秒>
```
**每次操作后 `sleep` 随机 1-10 秒**（不要固定间隔，模拟真人节奏）

#### 3. 评论（仅第 1 轮，随机 1-3 篇）

> **第 2 轮及以后跳过此步骤。**

随机选 1-3 篇帖子发表评论：
```
post_comment_to_feed(feed_id: '...', xsec_token: '...', content: '评论内容')
sleep <随机1-10秒>
```

#### 4. 查看通知（仅最后一轮，可选）

`get_notification_comments()` 查看新评论，有则回复 1-2 条。

## 评论铁律

- **用自己的人设说话**，不要模板化（禁止"写得好""学到了""干货满满"）
- **要有信息量**：补充数据、提出观点、分享经验，20-50 字为宜
- **不打广告、不引流、不荐股、不预测涨跌**
- **不评论竞品/敏感内容**
- 每条评论要基于帖子实际内容，像真人互动
- **回复自己帖子评论时**，走 `skills/xhs-comment-reply/SKILL.md` 流程（独立 skill，加载原帖上下文+人设风格）

## 工具注意事项

- **所有工具都不传 `account_id`**，身份由端口自动识别（传了会报错）
- 失败不重试，跳过即可

## 汇报

完成后用 `send_message` 向 `mag1` 汇报结果摘要：
- 点赞成功/失败几条
- 评论成功/失败几条
- 失败原因（如有）
- 消息末尾加 `[NO_REPLY_NEEDED]`
- mag1 回复了不要再回复，只汇报这一次
