...
| 平台 | 当前状态 | 核心 skill |
|------|----------|------------|
| 小红书 | 已运营 | xhs-content-planner、xhs-topic-collector、xhs-browser-publish |
...
- **素材巡逻**（xhs-topic-collector）：每日 3 次（10:00、15:00、21:00），巡逻小红书和外部信息源，积累选题素材
- **内容策划**（xhs-content-planner）：每日 3 次（9:00、13:00、18:00），从素材库挑选题推荐给小富龙
...
- **小红书内容运营 skill：**
  - **选题素材积累** — `skills/xhs-topic-collector/SKILL.md`：定时巡逻小红书和外部信息源，收集选题灵感写入素材库 `skills/xhs-content-planner/topic-library.md`。心跳触发或随时发现好素材时调用。
  - **内容策划与推荐** — `skills/xhs-content-planner/SKILL.md`：从素材库挑选题、生成内容、展示给小富龙确认后发布。心跳触发或小富龙说"推荐选题""今天发什么"时调用。
  - 两个 skill 的分工：topic-collector 负责"攒弹药"，content-planner 负责"选弹药+开枪"。
- **小红书相关一律走浏览器方式：**
