...
[每日 start_time 触发]
  ...
  → Step 4: 内容创作 + 生图
  → Step 5: 提交发布（走印务局）
  → Step 6: 记录 & 创建 1h 后 one-shot cron job
  ──── cron 触发（1h 后）────
  → Step 7: 数据回查 & 决策
    ├─ 有帖子 ≥ 500 阅读 → 当日完成 ✅
    ├─ 已发 5 次 → 当日完成（达上限）✅
    ├─ 最新帖 < 200 阅读 → 回到 Step 2 再发一条
    └─ 200 ≤ 阅读 < 500 → 再等 1h 后重新回查
  ──── cron 触发（23:00）────
...
### 6.3 创建 1h 回查 cron job

每次发帖后，**显式创建一个 one-shot cron 任务**，确保 1h 后一定触发回查：
...
> 使用 `--at "+1h"` 创建一次性 cron job（`schedule.kind: "at"`），执行后自动从 `cron/jobs.json` 中删除，不留垃圾。
...
elif 最新帖子 reads < 200 且距发帖已过 1h:
  → 回到 Step 2，发一条新帖
  → （循环继续）

elif 最新帖子 200 ≤ reads < 500:
  → 有潜力但还没破 500
  → 再创建一个 1h 后的 one-shot cron job 重新回查：
    openclaw cron add --name "auto-publish-check-<你的account_id>" --at "+1h" \
...
```
for each post in posts_today:
  if post.reads < 200 and post.note_id != "pending" and post.note_id != "lost":
    → 删除该笔记
...
| 印务局打回 | 读 `合规速查.md` 修改后重新提交，不额外计入发帖次数 |
