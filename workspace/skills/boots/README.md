...
这里住着量化策略相关的 skill。为了避免 skill 越用越臃肿 ("历史包袱"问题), 所有 boots(策略) 组的 skill 都遵循以下约定。
...
1. **skill 目录不住代码**: `workspace/skills/boots/<skill>/` 只放 SKILL.md / skill.json / references/ / memory/experiments/. 实际代码放 `/scripts/<skill>/`, SKILL.md 通过命令示例指向那里.
